...
export default function CartItemDisplay({ item }: CartItemDisplayProps) {
  ...
  return (
    <div className="flex items-center space-x-4 py-4 border-b border-border/60 animate-fade-in">
      ...
      <p className="font-semibold w-20 text-right text-primary">${(item.price * item.quantity).toFixed(2)}</p>
      <Button variant="ghost" size="icon" onClick={() => removeFromCart(item.id)} className="text-muted-foreground hover:text-destructive" aria-label="Remove item">
        <X className="h-5 w-5" />
      </Button>
    </div>
  );
